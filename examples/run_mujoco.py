...
from src.waypoint_tracker import WaypointTracker
...
def build_cost_matrices():
    Q = np.diag([
        30, 30, 120,    # posición (menos agresivo en Z)
        15, 15, 20,       # velocidad
        80, 80, 10,   # orientación
...
def run(
    ...
    waypoints = [
        np.array([0.0, 0.0, ROBOT_HIP_HEIGHT]),
        np.array([1.0, 0.0, ROBOT_HIP_HEIGHT]),
        np.array([1.0, 1.0, ROBOT_HIP_HEIGHT]),
        np.array([0.0, 1.0, ROBOT_HIP_HEIGHT]),
    ]

    tracker = WaypointTracker(waypoints)
    foot_nominal = {
        0: np.array([ 0.2,  0.1, -0.3]),  # FL
        1: np.array([ 0.2, -0.1, -0.3]),  # FR
        2: np.array([-0.2,  0.1, -0.3]),  # RL
        3: np.array([-0.2, -0.1, -0.3]),  # RR
    }
    ...
    try:
        for step in range(n_steps):
            ...
            if phase < 0.5:
                contact = np.array([1, 0, 0, 1], dtype=bool)  # FL + RR
            else:
                contact = np.array([0, 1, 1, 0], dtype=bool)  # FR + RL
            ...
            for i in range(4):
                foot_targets[i] = foot_nominal[i].copy()
                ...
                if not contact[i]:
                    ...
                    phase_swing = swing_phase

                    # avance
                    foot_targets[i][0] += step_length * (phase_swing - 0.5)

                    # levantar pata
                    foot_targets[i][2] += swing_height * np.sin(np.pi * phase_swing)
            ...
            if step % ctrl_steps == 0:
                try:
                    A_c_new, B_c_new = dyn.continuous_AB(x, contact, r_feet)
                    A_d_new, B_d_new = dyn.discretize(A_c_new, B_c_new)
                    g_d_new = dyn.gravity_vector()

                    if controller_name == "lqg":
                        controller.A_d = A_d_new
                        controller.B_d = B_d_new
                        controller.g_d = g_d_new

                        y = x
                        current_grfs = controller.step(y, x_ref, u_ref)
                        current_grfs = np.clip(current_grfs, -40, 40)

                    elif controller_name == "mpc":

                        if step % (ctrl_steps * 5) == 0:
                            controller.update_dynamics(A_d_new, B_d_new, g_d_new)
                        ...
                        current_grfs = np.clip(current_grfs, -60, 60)
            ...
            for i, leg in enumerate(["FL","FR","RL","RR"]):
                q_idx = env.legs_qpos_idx[leg]
                dq_idx = env.legs_qvel_idx[leg]
                tau_idx = env.legs_tau_idx[leg]

                q = env.mjData.qpos[q_idx]
                dq = env.mjData.qvel[dq_idx]
                ...
                if not contact[i]:
                    ...
                    lift = 0.08 * np.sin(np.pi * swing_phase)

                    q_des[1] -= lift
                    q_des[0] += 0.05 * np.cos(np.pi * swing_phase)

                tau_leg = np.clip(Kp*(q_des - q) - Kd*dq, -20, 20)
